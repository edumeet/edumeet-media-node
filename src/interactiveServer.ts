...
import { Consumer, DataConsumer, DataProducer, Producer, Router, Transport, Worker, WorkerLogLevel, WorkerLogTag } from 'mediasoup/types';
...
const runMediasoupObserver = () => {
	mediasoup.observer.on('newworker', (worker: Worker) => {
...
export const interactiveServer = (
	...
	try {
		...
		logger.error({ err: error }, 'interactiveServer() failed');
	}
